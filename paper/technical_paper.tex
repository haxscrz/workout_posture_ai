...
\section{Evaluation}
The repository ships with a pretrained model reporting 97.4\% validation accuracy, computed from the training pipeline's 20\% validation split and reported in the project README for the bundled model. This provides a strong baseline for seven-class discrimination on the curated training set. The repository does not publish dataset size or per-class metrics, so broader validation and more detailed reporting remain future work.
